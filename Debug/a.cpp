...
bool Showhelp()
{
	printf("ls        Display all the files the current directory has.\n");
...
bool Create_File(char file_name[],int index_node)   //对于Buffer数组，记得将具体数据后一个字节赋0，不然计算字节数时会出错
{
		                                                ...
		                                                //512-128-1=383个
	{
		...
		iNode[inode_num].disk_address[12] = spare_datablk_num[0];
		...
		int k;
		for(k=0;k<index_count-1;k++)
			Fill_Index_In_Datablk(index[k],128);
		Fill_Index_In_Datablk(index[k],buf_block_count-128*(index_count-1));*/
		/*---------------  二级间址暂时用不上  ------------------*/
	}
	/*----------------------再将填写好的iNode结构写回文件中----------------------------*/
	file = fopen(file_system_name,"rb+");
...
bool Copy_File(char filename1[],char filename2[])
{
	...
	if(!Create_File(file_name,dir_file_index))    //创建的文件文件名和文件应在的目录的目录文件的索引号
		return false;
	return true;
}
